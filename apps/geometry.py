...
class GeometryLoss:
        ...
        for idx, node in enumerate(self.iterate_nodes()):
            ...
            if abs(sm) < 1e-2:
                self.smooth_nodes.append(
                    (node, ((t0.norm()/self.segment_approx_length(node[0], pathObj)).item(), (t1.norm()/self.segment_approx_length(node[1], pathObj)).item())))
    ...
    def segment_approx_length(self, segment, pathObj):
        if segment[0] == 0:
            ...
            idxs = self.segList[segment[0]][segment[1]]
            # should have a pair of indices now
            length = (pathObj.points[idxs[1], :] - pathObj.points[idxs[0], :]).norm()+(pathObj.points[idxs[2], :] -
                                                                                       pathObj.points[idxs[1], :]).norm()+(pathObj.points[idxs[3], :] - pathObj.points[idxs[2], :]).norm()  # noqa
            return length
